...
- The server should make a request to the following endpoint at [CHUCKNORRIS.IO](https://api.chucknorris.io/): `https://api.chucknorris.io/jokes/random`
...
- The list of categories is available at this endpoint:
...
- "Build" the categories as radio inputs within the form. Setting the `name` attribute on the inputs to `category` will result in the selected category being submitted as a query string and available on the server as `req.query.category`.
    ...
    [Possible Solution](../../../labs/unit-2-full-stack/chuck-norris-solution/README.md)
    

![](https://i.imgur.com/nVr5KUi.png)