...
2. Sync your code with the starter code for this lesson (mandatory):
    ...
    ```
    git reset --hard origin/sync-17-oauth-starter
    ```
...
function resetReviews(){
    Movie.updateMany({},{reviews: []}).then(console.log).finally(process.exit)
}
...
- **access token**: A temporary key that provides access to an *owner's* *resources*.
...
![](https://i.imgur.com/OjjoaRx.png)

### Step 1.2 - Create a Project

- Click **Select a project**:

![](https://i.imgur.com/XuqQCLu.png)

- Click **NEW PROJECT**:

![](https://i.imgur.com/ylSC8mW.png)

- Update the **Project name** to something like `mongoose-movies`, then click **CREATE**:

![](https://i.imgur.com/dgKSFOG.png)
...
![](https://i.imgur.com/h9MbE5z.png)

- Click the **PUBLISH APP** button:

![](https://i.imgur.com/uI9QDhX.png)

- Click the **CONFIRM**:

![](https://i.imgur.com/ztX7wvL.png)

- Awesome. Now let's go view our credentials by clicking **Credentials** in the side menu:

![](https://i.imgur.com/Wbu1uKR.png)

- Click the **Web client 1** link:

![](https://i.imgur.com/gjK4cEk.png)
...
Implementing OAuth is complex. 

There are redirects going on everywhere, access tokens that only last for a short time, refresh tokens used to obtain a fresh access token, etc.
...
[Passport's website](http://passportjs.org/) states that it provides *Simple, unobtrusive authentication for Node.js*.
...
As soon as we write to the session a cookie named `connect.sid` will confirm that that the session middleware is doing its job.
...
**`git reset --hard origin/sync-18-sessions`**
...
**`git reset --hard origin/sync-19-passport`**
...
![](https://i.imgur.com/TPzABUk.png)

**versus**

![](https://i.imgur.com/0tt4eu3.png)
...
**`git reset --hard origin/sync-20-login-out`**
...
### 👉 You Do - Hide/Show Review `<form>` (5 minutes)
...
- Solution
    ...
    <!-- Add the line below -->
    <%}else { %>
            <h2>Please <a href="/auth/google">Login</a> to Add a Movie</h2>
    <%} %>
...
**`git reset --hard origin/sync-21-first-user-story`**
...
**`git reset --hard origin/sync-22-finish`**
