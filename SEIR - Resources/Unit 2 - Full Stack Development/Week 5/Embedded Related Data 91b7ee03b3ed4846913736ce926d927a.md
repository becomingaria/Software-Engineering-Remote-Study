...
Because only a single collection can be queried at a time, this approach may requires two separate database queries if we want access to both a movie and its reviews.

This is the reason *embedding* is considerably more efficient...
...
Here's what an ERD that models the `Movie --< Review`<br>relationship might look like:

![](https://i.imgur.com/NqDxv0G.png)
...
A touch more styling — **add** this new CSS to the bottom:
...
function create(req, res, next) {
    ...
    .then(function(movie){
				// you can modify any document's property 
        movie.reviews.push( req.body )
				// after a change occurs, the save method will return a promise
        return movie.save()
				// the document's save method returns a promise. 
    })
    .then(function(movie){
		
				// the .save() promise is thenable
				// any .then() callback that returns a value allows one to "chain" another .then()
				// after the document is updated we will redirect back to the movie detail page.
...
Movie.findOne({'reviews._id': req.params.reviewId})
.then(function(movie) {
