...
2. Clone the repo containing the starter code:
    ...
    ```
    git clone < link provided by instructional team >
...
Cat.create({name: 'Morris', breed: 'Orange Tabby'}).then(function(catDoc){
	console.log(catDoc)	
});
...
Now that we have a model, we're ready to perform some CRUD. First up is **creating** data!
...
1. `Model.create(<object or array of objects>)`
...
function create(req, res) {
  ...
  // split cast into an array if it's not an empty string - using a regular expression as a separator
	...
	if(req.body.cast) req.body.cast = req.body.cast.split(',').map(function(cast){return cast.trim()};
...
## 14. 💪 Implement `index` Functionality -  (20 mins)

 **Practice Exercise - Implement the movies `index` functionality that displays the list of movies**
...
## 15. ✨ || 💪 Defining Default Values for a Property
...
Let’s review the `create` action, this error will result in the `new.ejs` view being rendered instead of redirecting to the movies list:
