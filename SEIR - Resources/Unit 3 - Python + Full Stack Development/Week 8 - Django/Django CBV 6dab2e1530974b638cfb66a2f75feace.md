...
**Django views are functions defined in [views.py](http://views.py/) that run when its mapped route matches the HTTP request.**

Class-based Views are classes defined in the Django framework that we can extend and use instead of *view functions*.

For example, let's say we wanted to implement the *index* (view all) functionality for a Books data resource:
...
Then, in [**urls.py**](http://urls.py/), we invoke the `as_view()` class method of the `BookList` CBV which returns the view function for the route:
...
- Create a Django [ModelForm](Django%20CBV%206dab2e1530974b638cfb66a2f75feace.md) used to automatically create the form's inputs for the Model.
...
<img src="[https://i.imgur.com/DPxY8um.png](https://i.imgur.com/DPxY8um.png)">
...
Let's not forget to import `reverse` in [**models.py**](http://models.py/):
...
Let's add the two new routes in [**urls.py**](http://urls.py/):
...
We've referenced the new `CatUpdate` and `CatDelete` views in the routes so we need to add them to [**views.py**](http://views.py/).
...
[https://i.imgur.com/RS07hCP.jpg](https://i.imgur.com/RS07hCP.jpg)