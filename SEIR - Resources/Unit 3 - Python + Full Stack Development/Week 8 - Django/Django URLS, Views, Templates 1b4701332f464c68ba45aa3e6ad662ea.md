...
Let's include it as part of the Cat Collector project by adding it to the `INSTALLED_APPS` in [**settings.py**](http://settings.py/):
...
![](https://i.imgur.com/RozMgJ0.png)

### Connecting to the Database

Earlier we created a dedicated `catcollector` PostgreSQL database.

A Django project's configuration lives in [**settings.py**](http://settings.py/). Let's update it to use our `catcollector` database:
...
In order for us to interact with our Postgres database, we’ll need to install a package!
...
4. `git remote add origin https://github.com/YOUR_USERNAME/catcollector.git`
5. `git fetch --all`
...
In Django, routes are defined within modules named [**urls.py**](http://urls.py/).
...
Start by setting up **main_app**'s own [**urls.py**](http://urls.py/):

1. Create the [**urls.py**](http://urls.py/) module:
...
2. Include it in the project's **catcollector/urls.py**
    ...
    Each item in the `urlpatterns` list defines a URL-based route or, as in the case above, mounts the routes contained in another [urls.py](http://urls.py/) module.
...
4. Code the `about` view function in [**views.py**](http://views.py/) so that it renders a template named `about.html`
...
At the bottom of [**settings.py**](http://settings.py/), there is a `STATIC_URL = 'static/'` variable that declares the name of the folder that will contain static files within Django apps.
...
<img src="[https://i.imgur.com/ChPjvKm.png](https://i.imgur.com/ChPjvKm.png)">
...
<img src="[https://i.imgur.com/4DZVGiI.png](https://i.imgur.com/4DZVGiI.png)">
...
We only have a single [**views.py**](http://views.py/), so by naming the view `cat_index` we're anticipating that there might be another *index* view for a different resource in the future (*toys* maybe?).
...
Let's code the `cats_index` view inside of [**views.py**](http://views.py/):
...
In the next lesson, we'll create a `Cat` model but for now we're simply going to use a Python list containing a couple of "cat" dictionaries near the top of [**views.py**](http://views.py/):
...
<img src="[https://i.imgur.com/iohZqCp.png](https://i.imgur.com/iohZqCp.png)">