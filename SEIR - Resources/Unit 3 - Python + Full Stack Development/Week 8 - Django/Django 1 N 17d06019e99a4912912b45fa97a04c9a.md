...
Use a custom  to generate form inputs for a Model
...
Now let's import everything [**models.py**](http://models.py/) has to offer:
...
We could define the ModelForm inside of the [**models.py**](http://models.py/) module, but we're going to follow a best practice of defining it inside of a [**forms.py**](http://forms.py/) module instead:
...
Here's the updated `cats_detail` view function in [*views.py*](http://views.py/):
...
![](https://i.imgur.com/KGb0FcW.png)

Unfortunately, the *Feeding Date* field is just a basic text input. This is what Django uses by default for `DateField`s.
...
Let's start by stubbing up an `add_feeding` view function in [**views.py**](http://views.py/) so that the server will start back up and we can inspect our `<form>`:
...
![](https://i.imgur.com/3TEZ8nl.png)

And when the cat has been completely fed for the day:

![](https://i.imgur.com/E1yCDHr.png)
...
Be sure to add the import at the top of [*models.py*](http://models.py/).
