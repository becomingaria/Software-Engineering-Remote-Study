...
Use Django's  to Implement a M:M Relationship
...
![](https://i.imgur.com/imTYIBl.png)

Each row in the join table contains *foreign keys* for the other two tables' *primary keys*.
...
Now let's import everything from [**models.py**](http://models.py/):
...
> As a User, when viewing the detail page of a cat, I want to see a list of toys the cat has.
> 

and

> As a User, when viewing the detail page of a cat, I want to be able to add a toy from a list of toys that the cat doesn't already have.
> 
...
In [**views.py**](http://views.py/) update `cats_detail as follows`:
...
Let's first add a new routes with a URL pattern that includes both `id`s in [**urls.py**](http://urls.py/):
...
> AAU, when viewing the detail page for a cat, I want to be able to remove a toy from that cat
> 
...
For more information regarding *many-to-many through relationships*, start [here](https://docs.djangoproject.com/en/4.1/topics/db/models/#intermediary-manytomany) in the docs.
