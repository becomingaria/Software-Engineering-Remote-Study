# Django Uploading Image

![](https://i.imgur.com/r4VOQOq.jpg)
...
Use a file for environment variables/secrets
...
Use HTML to send a file to the server
...
8. Update the *detail.html* Template
9. Test it Out!
...
This lesson continues to build-out Cat Collector right where the *Django Many-to-Many Relationships* lesson left off.
...
Let's add the following code at the top of  [**settings.py**](http://settings.py/) (below the other imports near the top of the file is fine):
...
![](https://i.imgur.com/KOraf1L.png)

Then sign in:

![](https://i.imgur.com/c38jT6s.png)
...
Here's the high-level ERD:  `Cat -----< Photo`<br>***A Cat has many Photos*** and ***A Photo belongs to a Cat***

### Define the `Photo` Model

To [*models.py*](http://models.py/) we go:
...
The server currently shows an error because we've referenced an `add_photo` *view function* that doesn't exist.  Let's take care of that next...
...
Open up [**views.py**](http://views.py/) and let's do this...
...
⚠️ **Be sure to choose the endpoint for the region you used when creating your bucket.** Add the endpoint's URL to the `.env` file, for example:
...
> 👀 The for...empty template tag avoids having to wrap a for...in loop!
> 

Let's see how it looks:

![](https://i.imgur.com/S02DDIX.png)

Since we haven't uploaded any photos yet, we are seeing the *No Photos Uploaded* `<div>` as expected thanks to the `{% empty %} tag`.
...
Update the [*admin.py*](http://admin.py/) file to import the `Photo` Model and register it:
