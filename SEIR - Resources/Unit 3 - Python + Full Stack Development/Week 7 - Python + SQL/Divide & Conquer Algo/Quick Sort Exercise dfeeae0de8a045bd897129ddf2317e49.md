...
1. Define your base case - is the array argument has a length of one or zero → return array 
2. Determine a `partition` - select a random value (first, last, middle position) in the current array and remove it - we will use the last element.
3. Create a sub-section `left` with all values of that are smaller than your current `partition`
4. Create a sub-section `right` with all values greater than your current partition.
5. return a new array composed of the  sorted `left` array, `partition`, and (sorted) `right` arrays  
Hint: you can use [].concat() or the spread operator to position the new sorted values)

## Quick Sort Starter Code
