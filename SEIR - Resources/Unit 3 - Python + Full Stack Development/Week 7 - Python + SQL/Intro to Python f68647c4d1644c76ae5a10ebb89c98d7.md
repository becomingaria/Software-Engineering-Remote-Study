...
quotient = 5 // 2
quotient
2 # 2 is printed, because the decimal ".5" is truncated
```
...
Although we can access individual characters in a string, we cannot update the individual characters because strings are immutable (like JS):
...
- (2) Will the above code run without error in Python?
    ...
    No. Error: TypeError: can only concatenate str (not "int") to str
