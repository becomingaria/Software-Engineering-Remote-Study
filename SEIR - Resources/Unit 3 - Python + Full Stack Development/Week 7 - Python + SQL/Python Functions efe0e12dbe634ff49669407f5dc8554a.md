...
- `cd ~/sei/lessons && mkdir py_functions && cd py_functions`
- `touch functions.py`
- `code .`
...
- (1) Which line of code within the above `get_category()` function will be the last to execute?
...
**Which of the following statements will result in an error? (there could be more than one)**
A) `add(10, 100.)` 
B) `add(10, '10')` 
C) `add(100)` 
D) `add('abc', 'def')` 
