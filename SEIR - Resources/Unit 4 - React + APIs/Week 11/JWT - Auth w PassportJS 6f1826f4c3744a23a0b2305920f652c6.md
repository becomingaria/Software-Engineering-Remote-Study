...
2. Create a new file in a `middleware` directory called `auth.js`.
3. Add a secret to a .env file; example: `JWT_SECRET=Thisisasecretformytests`
4. Add the following code below to `auth.js`
...
3. We will also export a middleware called `requireToken` that we can add to our routes. This middleware can restrict access to authenticated users. 
...
4. Update the `/login` route as follows
...
2. Add `requireToken`to the de-structured require statement  from auth.
...
## Authorizing access to Create, Update, Destroy people routes

Update the POST, PUT and DELETE routes as follows:
...
// CREATE
// POST api/people
router.post("/", requireToken, peopleCtrl.create);

// UPDATE
// PUT api/people/5a7db6c74d55bc51bdf39793
router.put("/:id", requireToken, peopleCtrl.update);

// DESTROY
// DELETE api/people/5a7db6c74d55bc51bdf39793
router.delete("/:id", requireToken, peopleCtrl.delete);

```

Next, open the `controllers/people-controller.js` and import your PassportJS authorization utility function.

```jsx
const { handleValidateOwnership } = require("../middleware/auth");
```

## Validating ownership for write with PassportJS

Update the `People` POST, PUT and DELETE routes as follows:
...
router.get( "/logout", requireauthCtrl.logout)
```
...
export async function logout (req, res, next) {
  try {
    res.status(200).json({
      token: ""
			// this null value will be accessed by an auth utility service 
    })
...
## Next: [React](JWT%20Authentication%20d37761dea0b94c3889df895cf98a2e0f.md) [Authentication & Authorization](https://www.notion.so/9bc07256a4e74d20a8232b9051d6a0e8?pvs=21)