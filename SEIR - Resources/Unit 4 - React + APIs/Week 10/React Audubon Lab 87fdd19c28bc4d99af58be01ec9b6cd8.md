...
1. Fork and clone [this repository.](https://github.com/SEIR-221-Resources/react-audubon-site-lab)
2. Change into the new directory with `cd react-audubon-site-lab`.
3. Install the project dependencies with `npm i` or `npm install`.
4. Fulfill the requirements below to complete the assignment.
...
To set up your project to use React Router, you'll need to install it, import the BrowserRouter component, and then use that component to wrap the App component in the index.js file in the same way that we did it during the [lecture](../../../lectures/react-router/README.md#importing-dependencies).
...
Now, you can add your fetch request inside the curly braces of the callback function. The url for the birds data is: `http://localhost:3001/birds` (served locally via `json-server` — see the `data/birds.json` file in the lab repo). Use the fetch pattern from the [AJAX & APIs lecture](../../lectures/apis-ajax/README.md#the-api-response) for your fetch call:
...
The browser should still display all of the birds. However, if you navigate to a different route, such as [http://localhost:3000/details](http://localhost:3000/details), the component no longer displays!
...
return (
    <div className='details-container'>
        ...
        <div className='details'>
            ...
            <p>
                Would be vulnerable to loss of habitat, but no significant
                decline noted so far. In some regions, Brown-headed Cowbirds
                often lay eggs in nests of this species.
            </p>
            <a
...
Now if you navigate to the details route in the browser by going to: [http://localhost:3000/details](http://localhost:3000/details), you should see the BirdDetails component display.
...
This will mean that if we try and navigate to [http://localhost:3000/details](http://localhost:3000/details) now, it will no long match the route path because there's nothing after "details" in the URL. However, if we navigate to [http://localhost:3000/details/a](http://localhost:3000/details/a), it now matches the _pattern_ in the `path` attribute of one of the Route components!
...
Next, we need to add the fetch call into the useEffect callback just as we did earlier, except in this case the url is going to be: [https://ga-audubon-api.herokuapp.com/api/birds](https://ga-audubon-api.herokuapp.com/api/birds) + the bird's id and instead of using `setBirds` to put the json data into state, we'll use the `setBird` method here.

How can we get the id of the bird? The react-router [useParams()](https://v5.reactrouter.com/web/api/Hooks/useparams) hook.

<aside>
💡 From the Docs:  `useParams` returns an object of key/value pairs of URL parameters. Use it to access `match.params` of the current `<Route>`.

</aside>

By accessing the id of the bird in from the URL path, we are able to fetch the bird’s details from the API. After the bird is found you should call `setBird` .
...
```jsx
// in your BirdDetails component function body (before the return)
const { id } = useParams()
const url = `https://audubon-api.herokuapp.com/api/birds/${id}`
...
Finished everything above and want to stretch your brain? Try adding a create bird page.
...
Make sure you add the functionality so that when the user submits the form, it will "add" the new bird to the "database" of birds stored in App.js. This will require a structural refactor, including passing local bird data from `App` > `Birds` page through props. Additionally, an ‘addBird’ function will need to be passed to the `CreateBird` page to provide formData to your App’s local birds array. Additional re-factors may be required to tie the two data stores together.

## Plagiarism

Take a moment to refamiliarize yourself with the [Plagiarism policy](../../Plagiarism 077d07f1283d4f0899f7974598de15ba.md). Plagiarized work will not be accepted.
