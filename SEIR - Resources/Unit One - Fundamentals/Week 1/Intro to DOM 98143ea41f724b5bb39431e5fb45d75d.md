...
Yes, we can select DOM elements using CSS3 selectors just like we do to target elements for styling!
...
2. Add some content inside of the `<p>` tag - try typing `lorem [tab]` to emit (using *emmet*) random *lorem ipsum* text.
...
3. In the JS, write the line of code that will add an `href` attribute that will make the link navigate to "[https://www.google.com](https://www.google.com/)".

> Hint: Which of the attribute methods above looks tasty?
> 

## 10. The `classList` Attribute
