...
Functions may be written to accept data as input.  

The data passed to a function when called is known as **arguments**.
...
Write a function named `planetHasWater` as a **function expression**. It will have the following functionality: 

1. It will have one parameter, `planet`, that accepts a string argument.
2. Return `true` if the `planet` argument is either 'Earth' or 'Mars', otherwise return `false`.
3. **Bonus:** Make the function work regardless of the casing of the planet string being passed in ('earth', 'MARS', etc.).
