...
jQuery's motto is:<br>*"write less, do more"*
...
To use any JavaScript library in a web application, we need to ensure that the script files are loaded in *index.html*.
...
Let's use a CDN to load the jQuery library in the `<head>` of Connect-Four's *index.html*:
...
- (2) The jQuery Object contains an array of zero or more vanilla _______________.
